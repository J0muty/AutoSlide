...
function openInsertModal(idx, editing = false){
    ...
    input.addEventListener('input',()=> input.value.trim() ? create.classList.add('enabled') : create.classList.remove('enabled'));
    modal.addEventListener('keydown',e=>{ if(e.key==='Enter' && input.value.trim()) create.click(); });
    ...
    create.addEventListener('click',()=>{
        ...
        if(editing){
            currentLines[idx] = input.value.trim();
        } else {
...
function toggleSlideMenu(item, idx) {
    ...
    menu.addEventListener('click', e=>{
        ...
        if(act === 'delete'){ currentLines.splice(idx,1); renderSlides(currentLines.join('\n')); }
